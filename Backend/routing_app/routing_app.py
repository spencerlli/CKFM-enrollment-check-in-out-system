...
AMIS_RES_TEMPLATE = {"status": 0, "msg": None, "data": {}}
...
def guardianSignUp():
    ...
    if (
        requests.get(
            REST_API + "/guardian/phone/%s" % guardian_json["phone"]
        ).status_code
        == 200
    ):
    ...
    if request.method == "GET":
        ...
        if "phone" in request.json and "pwd" in request.json:
            ...
            if guardian_query.status_code == 200:
                ...
                if (
                    "phone" in guardian_json.keys()
                    and guardian_json["pwd"] == pwd
                    and (
                        not guardian_json["pwd_hash"]
                        or (
                            guardian_json["pwd_hash"]
                            and bcrypt.check_password_hash(
                                guardian_json["pwd_hash"], pwd
                            )
                        )
                    )
                ):
                    ...
                    if guardian_json.get("fname"):
                        res.set_cookie(
                            key="fname",
                            value=guardian_json["fname"],
                            expires=COOKIE_EXPIRE_TIME,
                        )
                        res.set_cookie(
                            key="lname",
                            value=guardian_json["lname"],
                            expires=COOKIE_EXPIRE_TIME,
                        )
                    else:
                        res.set_cookie(
                            key="fname", value="New", expires=COOKIE_EXPIRE_TIME
                        )
                        res.set_cookie(
                            key="lname", value="Guardian", expires=COOKIE_EXPIRE_TIME
                        )

                    family_json = requests.get(
                        REST_API + "/family/guardian/%d" % guardian_json["id"]
                    ).json()
                ...
                # if hash password is set, check it; otherwise check the plain text password
                if "phone" in teacher_json.keys() and (
                    not teacher_json["pwd_hash"]
                    or (
                        teacher_json["pwd_hash"]
                        and bcrypt.check_password_hash(teacher_json["pwd_hash"], pwd)
                    )
                ):
                    res_json["msg"] = "Logged in successfully!"
                    res = jsonify(res_json)
                    ...
                    res.set_cookie(
                        key="fname",
                        value=teacher_json["fname"],
                        expires=COOKIE_EXPIRE_TIME,
                    )
                    res.set_cookie(
                        key="lname",
                        value=teacher_json["lname"],
                        expires=COOKIE_EXPIRE_TIME,
                    )
                    ...
                    if teacher_json["privilege"] == 0:
                        ...
                        classes_json = requests.get(
                            REST_API + "/classes/teacher/%d" % teacher_json["id"]
                        ).json()
                        ...
                        if classes_json and len(classes_json) > 0:
                            ...
                            res.set_cookie(
                                key="classes_id",
                                value=str(classes_id),
                                expires=COOKIE_EXPIRE_TIME,
                            )
                        res.set_cookie(
                            key="user_id",
                            value=str(teacher_json["id"]),
                            expires=COOKIE_EXPIRE_TIME,
                        )
...
def enrollFamily():
    ...
    for i, student in enumerate(student_json):
        student_json[i]["barcode"] = (
            "s"
            + student["fname"][0].upper()
            + student["lname"][0].upper()
...
def adminManage(object):
    ...
    if object != "classes":
        if (
            request.method == "GET"
        ):  # guardian / student / familyInfo / family / teacher
            ...
            if object == "family":
                ...
                guardian_id_to_json = {
                    guardian["id"]: guardian for guardian in guardians_json
                }
                student_id_to_json = {
                    student["id"]: student for student in students_json
                }
                ...
                for i, family in enumerate(object_json):
                    ...
                    object_json[i]["guardian_name"] = (
                        guardian_json["fname"] + " " + guardian_json["lname"]
                    )
                    object_json[i]["guardian_phone"] = guardian_json["phone"]
                    object_json[i]["guardian_email"] = guardian_json["email"]
                    object_json[i]["guardian_relationship"] = guardian_json[
                        "relationship"
                    ]
                    object_json[i]["student_name"] = (
                        student_json["fname"] + " " + student_json["lname"]
                    )
            ...
            if object == "familyInfo":
                ...
                for i, _ in enumerate(object_json):
                    (
                        object_json[i]["guardian_names"],
                        object_json[i]["student_names"],
                    ) = (set(), set())

                id_familyInfo = {
                    familyInfo["id"]: familyInfo for familyInfo in object_json
                }
                ...
                for id, _ in id_familyInfo.items():
                    id_familyInfo[id]["guardian_names"] = ", ".join(
                        list(id_familyInfo[id]["guardian_names"])
                    )
                    id_familyInfo[id]["student_names"] = ", ".join(
                        list(id_familyInfo[id]["student_names"])
                    )
            ...
            if object == "student":
                ...
                for teacher in teachers_json:
                    ...
                    classes_teacher_name[teacher["classes_id"]].append(
                        teacher["fname"] + " " + teacher["lname"]
                    )

                for i, student in enumerate(object_json):
                    if student["classes_id"]:
                        object_json[i]["teacher_name"] = classes_teacher_name[
                            student["classes_id"]
                        ]
            ...
            if request.method == "PUT":
                ...
                requests.put(
                    REST_API + "/" + object + "/" + request.args.get("id"),
                    json=request.json,
                )
            ...
            if object == "guardian" or object == "student":
                requests.delete(
                    REST_API + "/family/" + object + "/" + request.args.get("id")
                )
                requests.delete(
                    REST_API + "/log/" + object + "/" + request.args.get("id")
                )
            if object == "teacher" or object == "student":
                requests.delete(
                    REST_API + "/classes/" + object + "/" + request.args.get("id")
                )
            if object == "familyInfo":
                family_json = requests.get(
                    REST_API + "/family/" + request.args.get("id")
                ).json()
        ...
        if request.method == "GET":
            ...
            res["data"]["options"] = [
                {"label": classes_id, "value": classes_id}
                for classes_id in classes_id_set
            ]
            res["msg"] = "Successfully get data!"
        elif request.method == "PUT" or request.method == "POST":
            if request.method == "PUT":
                if request.json["operation"] == "add":
                    ...
                    for student in request.json["students"]:
                        ...
                        if student["classes_id"]:
                            ...
                            requests.put(
                                REST_API + "/student/%s" % student_id,
                                json={"classes_id": None},
                            )
                            requests.delete(
                                REST_API
                                + "/classes/%s/%s/%s"
                ...
                if len(teacher_name) != 2:
                    ...
                    res[
                        "msg"
                    ] = "Please correctly input teacher's first and last name, splited by one space."
                    return jsonify(res)
            ...
            classes_json = requests.get(
                REST_API + "/classes/%s" % request.args.get("id")
            ).json()
...
def userManage():
    res = {"status": 0, "msg": None, "data": {"items": [], "hasNext": False}}
    ...
    if request.method == "GET":
        ...
        if request.method == "POST":
            ...
            if (
                object == "guardian"
                and requests.get(
                    REST_API + "/phone/%s" % object_json["phone"]
                ).status_code
                == 200
            ):
                res["status"] = 1
                ...
                if object == "student":
                    object_json["barcode"] = (
                        object_json["fname"][0].upper()
                        + object_json["lname"][0].upper()
                        + generate_random_str(5)
                    )

                object_id = requests.post(
                    REST_API + "/%s" % object, json=object_json
                ).json()["id"]
            ...
            # change password
                ...
                requests.put(
                    REST_API + "/%s/%s" % (object, object_id), json=object_json
                )

                res[
                    "msg"
                ] = "Successfully change password! Please login with new password next time."
                return jsonify(res)

            requests.put(REST_API + "/" + object + "/" + object_id, json=request.json)
            ...
            if (
                object == "student"
                and requests.get(
                    REST_API + "/classes/%s/%s" % (object, object_id)
                ).status_code
                == 200
            ):
...
def preCheckIn():
    ...
    if request.method == "GET":
        ...
        for family in family_json:
            if family["guardian_id"] not in guardian_id_set:  # filter repeat
                guardian_json = requests.get(
                    REST_API + "/guardian/%d" % family["guardian_id"]
                ).json()
                family_list.append(
                    {
                        "object": "guardian",
            ...
            if family["student_id"] not in student_id_set:  # filter repeat
                student_json = requests.get(
                    REST_API + "/student/%d" % family["student_id"]
                ).json()
        ...
        for student_json in student_list:
            ...
            requests.post(REST_API + "/log", json=log_json)
            requests.put(
                REST_API + "/student/%s" % student_json["id"], json=student_json
            )

        # generate barcode for all guardians
        barcode = (
            request.cookies.get("fname")[0].upper()
        ...
        if query_res.status_code != 200:
            ...
            if int(student_json["status"]) != 1:
                ...
                last_log_json = requests.get(
                    REST_API + "/log/student/%s" % student_json["id"]
                ).json()
    ...
    if request.method == "POST":
        if not request.json.get("student_barcode"):  # scanned guardian barcode
            ...
            if guardian_query.status_code == 404:
                ...
                family_id = (
                    requests.get(REST_API + "/family/guardian/%d" % guardian_id)
                    .json()[0]
                    .get("id")
                )
                ...
                res.set_cookie(
                    key="guardian_id",
                    value=str(guardian_id),
                    expires=COOKIE_EXPIRE_TIME,
                )
                res.set_cookie(
                    key="family_id", value=str(family_id), expires=COOKIE_EXPIRE_TIME
                )
                return res
        else:
            ...
            if student_query.status_code == 404:
                ...
                requests.put(
                    REST_API + "/student/%d" % student_json["id"], json=student_json
                )
                requests.post(REST_API + "/log", json=log_json)
    ...
    if request.method == "GET":
        if user_group == "guardian":
            ...
            msgBoard_json = requests.get(
                REST_API + "/msgBoard/guardian/%d" % user_id
            ).json()
            for msg in msgBoard_json:
                if msg["sender_group"] == "teacher":
                    teacher_json = requests.get(
                        REST_API + "/teacher/%d" % int(msg["sender_id"])
                    ).json()
                    msg_show.append(
                        {
                            "id": msg["id"],
                            "fname": "Teacher - " + teacher_json["fname"],
                            "lname": teacher_json["lname"],
                            "msg": msg["content"],
                            "timestamp": msg["time"],
                        }
                    )
            ...
            msg_show = []
            msgBoard_json = requests.get(
                REST_API + "/msgBoard/teacher/%d" % user_id
            ).json()
            for msg in msgBoard_json:
                if msg["sender_group"] == "guardian":
                    guardian_json = requests.get(
                        REST_API + "/guardian/%d" % int(msg["sender_id"])
                    ).json()
                    msg_show.append(
                        {
                            "id": msg["id"],
                            "fname": "Guardian - " + guardian_json["fname"],
                            "lname": guardian_json["lname"],
                            "msg": msg["content"],
                            "timestamp": msg["time"],
                            "read": "Yes" if msg["been_read"] else "No",
                        }
                    )
                else:
                    msg_show.append(
        ...
        if user_group == "guardian":
            ...
            for student_id in student_id_list:
                ...
                classes_json = requests.get(
                    REST_API + "/classes/student/%d" % student_id
                ).json()
                ...
                msg_json = {
                    ...
                    "sender_group": user_group,
                    "about_student": student_id,
                    "content": request.json["msg"],
            ...
            if len(error_student_id_list) > 0:
                ...
                res["msg"] = (
                    "Student ID: "
                    + ", ".join(map(str, error_student_id_list))
            ...
            if "reply_msg_id" in request.json.keys():
                ...
                for reply_msg_id in reply_msg_id_list:
                    requests.put(
                        REST_API + "/msgBoard/%d" % reply_msg_id,
                        json={"been_read": True},
                    )
                ...
                for student_id in student_id_list:
                    family_json = requests.get(
                        REST_API + "/family/student/%d" % student_id
                    ).json()
                    for family in family_json:
                        msg_json = {
                            "sender_id": user_id,
                            "receiver_id": family["guardian_id"],
                            "sender_group": user_group,
                            "about_student": student_id,
                            "content": request.json["msg"],
                            "time": int(datetime.datetime.now().timestamp()),
        ...
        for classes in classes_json:
            if classes["student_id"] not in student_id_set:  # filter repeat
                if classes["student_id"] == -1:
                    continue
                student_json = requests.get(
                    REST_API + "/student/%d" % classes["student_id"]
                ).json()
                student_info_list.append(
                    {
                        "id": student_json["id"],
...
def log():
    ...
    if request.method == "GET":
        if request.cookies.get("user_group") == "teacher":
            classes_json = requests.get(
                REST_API + "/classes/%s" % request.cookies.get("classes_id")
            ).json()
        ...
        for log in logs_json:
            ...
            if (
                request.cookies.get("user_group") == "teacher"
                and student_id not in student_id_set
            ):
...
def guestEnroll():
    ...
    if request.method == "POST":
        ...
        for guardian in request.json["guardians"]:
            guardian["barcode"] = (
                guardian["fname"][0].upper()
                + guardian["lname"][0].upper()
                + generate_random_str(5)
            )
        ...
        for student in request.json["students"]:
            ...
            student["status"] = 1
            student_res = requests.post(REST_API + "/student", json=student)
    ...
    if f.filename.endswith(".xlsx"):
        ...
        if res_dict["status"] == 0:
            ...
            for _, row in guardian_df.iterrows():
                ...
                guardian_json["barcode"] = (
                    guardian_json["fname"][0].upper()
                    + guardian_json["lname"][0].upper()
                    + generate_random_str(5)
                )
            ...
            student_df["birthdate"] = student_df["birthdate"].apply(
            ...
            for _, row in student_df.iterrows():
                ...
                student_json["barcode"] = (
                    student_json["fname"][0].upper()
                    + student_json["lname"][0].upper()
                    + generate_random_str(5)
                )
                student_json["status"] = 1
                student_res = requests.post(REST_API + "/student", json=student_json)
            ...
            for guardian in guardian_list:
                for student in student_list:
                    ...
                    family_res = requests.post(REST_API + "/family", json=family)
